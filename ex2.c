...
typedef enum {
	VID1,VID2,VID2_IN_VID1,RED,GREEN,BLUE,BLACK_N_WHITE,NONE
}Choice;
...
typedef struct VideoState {
	...
	Choice color_choice;
	int channel;  //video channels
	int audioChannel;
	
} VideoState;

VideoState * channels[3];
 int channel_req;	//current channel
 int audio_req;		//current audio
static VideoPicture *global_vp1;	
static Choice choice;	//user choice
...
void audio_callback(void *userdata, Uint8 *stream, int len) {
	...
	VideoState *is = channels[audio_req];
	int len1, audio_size;
	double pts;
...
int queue_picture(VideoState *is, AVFrame *pFrame, AVFrame *pFrameRGB, double pts) {
	...
	if (vp->bmp) {
		...
		switch (is->color_choice) {
		case BLUE:
			//paint it blue
			for (k=0; k<is->video_st->codec->width*is->video_st->codec->height; ++k) {
		...
		/*copy pixels from vid 2 vid */
		if(is->channel==3)	
			global_vp1=vp;
		 if(choice == VID2_IN_VID1  && is->channel == 2)
			memcpy(vp->bmp->pixels,global_vp1->bmp->pixels,sizeof(*(global_vp1->bmp->pixels)));
		...
		sws_scale(is->sws_ctx, (const uint8_t * const *) pFrameRGB->data,
				...
				pict.linesize);
		//paint grayscale 
		if (is->color_choice == BLACK_N_WHITE) {
			memset(pict.data[1],128, is->video_st->codec->width*is->video_st->codec->height);
			memset(pict.data[2],128, is->video_st->codec->width*is->video_st->codec->height);
		}
...
int stream_component_open(VideoState *is, int stream_index) {
	...
	case AVMEDIA_TYPE_VIDEO:
		...
		else is->sws_ctx = sws_getContext(is->video_st->codec->width,
				is->video_st->codec->height, PIX_FMT_RGB24,
				is->video_st->codec->width, is->video_st->codec->height,
				PIX_FMT_YUV420P, SWS_FAST_BILINEAR, NULL, NULL, NULL);
